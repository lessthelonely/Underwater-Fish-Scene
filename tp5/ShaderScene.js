...
export class ShaderScene extends CGFscene {
	...
	init(application) {
		...
		this.testShaders = [
			...
			new CGFshader(this.gl, "shaders/blueYellowanim.vert", "shaders/blueYellowanim.frag"),
		...
		//Repetir o que está feito para a texture2.
		this.testShaders[12].setUniformsValues({ uSampler2: 2 });
		//Definir timeFactor.
		this.testShaders[12].setUniformsValues({ timeFactor: 0.1});
	...
	update(t) {
		...
		if (this.selectedExampleShader == 6)
			...
			this.testShaders[12].setUniformsValues({ timeFactor: t / 200 % 1000 });
	}
	...
	display() {
		...
		if (this.selectedExampleShader == 12)
			this.appearance.setTexture(this.texture4);
		else
			this.appearance.setTexture(this.texture);
